...
def response_path(path):
    ...
    ### Note: Need a beter understanding of the "Webroot" File
    file_path = os.getcwd() + "\webroot"
    
    # Create a list for comparison if provided file paths
    onlyfolders = []
    for (_ dirnames, _) in walk(file_path):
        onlyfolders.append(dirnames)
        break
    
    # Create Comparison list for Files
    onlyfiles = [file for file in os.listdir(file_path) if os.path.isfile(join(file_path, file))]
    ...
    #try:
        #mime_type = mime_dictionary['file extension']
    # Except KeyError:
        #raise NotImplementedError
